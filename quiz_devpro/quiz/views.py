...
def perguntas(requisicao, indice):
    ...
    try:
        pergunta = Pergunta.objects.filter(
            disponivel=True).order_by('id')[indice - 1]
    except IndexError:
        ...
        if requisicao.method == 'POST':
            ...
            if alternativa_escolhida == pergunta.alternativa_correta:
                try:
                    primeira_resposta = Resposta.objects.filter(
                        pergunta=pergunta).order_by('criacao')[0]
                except IndexError:
                    pontos = 100
                else:
                ...
                Resposta(aluno_id=aluno_id, pergunta=pergunta,
                         pontos=pontos).save()
